...
import instafel.app.managers.modals.ParseResult;
...
public class OverridesManager {
    ...
    public OverridesManager(Activity act) {
        ...
        if (debugModeState) {
            this.overrideFile = new File(act.getExternalFilesDir(null), "instafel_files/mc_overrides.json");
            this.mappingFile = new File(act.getExternalFilesDir(null), "instafel_files/example_mapping.json");
        } else {
    ...
    public long getMappingFileSize() throws IOException {
        if (mappingFile != null && mappingFile.exists() && mappingFile.isFile()) {
            return mappingFile.length();
        }
        return 0L;
    }
    ...
    public boolean createMappingFileDebug() {
        try {
            File folder = new File(act.getExternalFilesDir(null), "instafel_files");
            File iflCrashlogFile = new File(folder.getPath(), "example_mapping.json");
            if (!folder.exists()) {
                folder.mkdirs();
            }
    ...
    public boolean createOverrideFileDebug() {
        try {
